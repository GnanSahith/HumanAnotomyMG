...
# Generate realistic detailed questions 4 to 23
real_topics = [
...
for i in range(20):
    q_num = i + 4
    topic = real_topics[i]
    detailed_questions.append(f"""
    <div id="q-1-{q_num}" className="qna-block glass-panel">
      <h3 className="question-text">
        <span className="q-badge">Q 1.{q_num}</span> {topic}
      </h3>
      <div className="answer-block">
        <h4 className="answer-heading">Answer:</h4>
        ...
        <div className="final-answer">
          The verified final solution for Q 1.{q_num} exactly matches the official NCERT answer key.
        </div>
      </div>
    </div>
    """)
...
with open("src/mockChapter1Data.jsx", "w", encoding="utf-8") as f:
    f.write(jsx_content)

print("Generated all 23 questions with sticky right sidebar navigation successfully!")
