...
export default function CustomCircuitConstructionKitDCVirtualLab({ onBack, title }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col font-sans select-none">
      {/* Top Header Panel */}
      <header className="border-b border-slate-800/80 bg-slate-900/60 backdrop-blur-md px-6 py-4 flex items-center justify-between z-10 shadow-lg">
        <div className="flex items-center gap-4">
          <button 
            onClick={onBack}
            className="p-2 hover:bg-slate-800 border border-slate-700/50 rounded-xl transition-all text-slate-400 hover:text-white hover:scale-105 active:scale-95"
          >
            <ArrowLeft size={20} />
          </button>
          <div>
            <h1 className="text-xl font-bold bg-gradient-to-r from-cyan-400 via-sky-400 to-indigo-400 bg-clip-text text-transparent">
              {title || "DC Circuit Kit - Virtual Lab"}
            </h1>
            <p className="text-xs text-slate-400 mt-0.5">Interactive virtual laboratory for DC electronics simulations</p>
          </div>
        </div>
        
        {/* Top Control Panel */}
        <div className="flex items-center gap-4">
          <div className="flex bg-slate-800/80 p-0.5 rounded-lg border border-slate-700/50 shadow-inner">
            <button
              onClick={() => handleSetRepresentation('lifelike')}
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${representation === 'lifelike' ? 'bg-cyan-500 text-slate-950 shadow-md scale-102' : 'text-slate-400 hover:text-slate-200'}`}
            >
              Virtual Lab
            </button>
            <button
              onClick={() => handleSetRepresentation('schematic')}
              className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${representation === 'schematic' ? 'bg-cyan-500 text-slate-950 shadow-md scale-102' : 'text-slate-400 hover:text-slate-200'}`}
            >
              Schematic
            </button>
          </div>
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-2 px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700/80 text-slate-200 text-sm font-semibold rounded-lg border border-slate-700/50 transition-all hover:scale-103 active:scale-97"
          >
            <RotateCcw size={15} />
            Reset Lab
          </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-72 border-r border-slate-800/80 bg-slate-900/40 backdrop-blur-md p-5 flex flex-col justify-between overflow-y-auto">
          <div>
            ...
            <div className="mt-8 bg-slate-900/60 p-4 rounded-xl border border-slate-800/80">
        ...
        {/* Middle Area - Interactive Circuit Canvas */}
        <main className="flex-1 flex items-center justify-center p-4 bg-slate-950 relative">
          <div className="relative rounded-2xl border border-slate-850 overflow-hidden shadow-2xl bg-slate-900/20">
            <canvas
              ref={canvasRef}
              width={800}
              height={500}
        ...
        <aside className="w-80 border-l border-slate-800/80 bg-slate-900/40 backdrop-blur-md p-5 flex flex-col justify-between overflow-y-auto">
            ...
            <div className="flex border-b border-slate-800/60 pb-1.5 gap-4">
              {['inspector', 'tools', 'settings'].map(tab => (
                <button
                  ...
                  className={`text-xs font-bold uppercase tracking-wider pb-1 transition-all ${activeTab === tab ? 'text-cyan-400 border-b border-cyan-400' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  {tab}
                </button>
              ))}
                  ...
                  <div className="space-y-4 bg-slate-900/60 p-4 rounded-xl border border-slate-800/80">
                    ...
                    <div className="border-t border-slate-800/80 pt-3 mt-3 space-y-1.5 text-xs text-slate-400 font-medium">
                      ...
                      <div className="flex justify-between border-t border-slate-800/40 pt-1.5 mt-1.5">
                        ...
                        <span className="font-mono text-cyan-400">{(selectedComponent.current || 0).toFixed(3)} A</span>
                      </div>
                    </div>
                  </div>
                ) : selectedJunction ? (
            ...
            {activeTab === 'tools' && (
              <div className="space-y-4">
                <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800/80 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${voltmeterEnabled ? 'bg-cyan-500' : 'bg-slate-800'}`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${ammeterEnabled ? 'bg-cyan-500' : 'bg-slate-800'}`}
                    >
                ...
                <div className="space-y-3">
                  {voltmeterEnabled && (
                    <div className="bg-slate-900/30 p-3 rounded-lg border border-slate-850 flex justify-between items-center text-xs">
                  ...
                  {ammeterEnabled && (
                    <div className="bg-slate-900/30 p-3 rounded-lg border border-slate-850 flex justify-between items-center text-xs">
                      ...
                      <span className="font-mono font-bold text-cyan-400 text-sm">{ammeterValue.toFixed(2)} A</span>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800/80 space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${showElectrons ? 'bg-cyan-500' : 'bg-slate-800'}`}
                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${showValues ? 'bg-cyan-500' : 'bg-slate-800'}`}
                    >
          ...
          <div className="mt-8 bg-slate-950 p-4 rounded-xl border border-slate-850 text-xs text-slate-400 space-y-1.5 font-medium shadow-inner">
