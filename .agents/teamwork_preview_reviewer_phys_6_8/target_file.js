...
export default function CustomMassesAndSprings({ onBack, title }) {
    ...
    const renderSpringPath = (ps, s) => {
        return <path ref={el => springRefs.current[s.id] = el} d={getSpringPathStr(ps.y, s)} fill="none" stroke="rgba(255, 255, 255, 0.6)" strokeWidth="4" strokeLinejoin="round" />;
    };
                                ...
                                return (
                                    <div style={{ display: 'flex', alignItems: 'flex-end', height: '120px', gap: '12px', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '8px', paddingTop: '8px' }}>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                            <div ref={energyKeRef} style={{ width: '100%', height: `${keH}px`, background: '#32d74b' }} />
                                            <span style={{ fontSize: '10px', color: '#aaa' }}>KE</span>
                                        </div>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                            <div ref={energyPegRef} style={{ width: '100%', height: `${peGravH}px`, background: '#0a84ff', transform: rawPeGravH < 0 ? `translateY(${peGravH}px)` : 'none' }} />
                                            <span ref={energyPegLabelRef} style={{ fontSize: '10px', color: '#aaa', transform: rawPeGravH < 0 ? `translateY(${peGravH}px)` : 'none' }}>PEg</span>
                                        </div>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                            <div ref={energyPesRef} style={{ width: '100%', height: `${peSprH}px`, background: '#00f0ff' }} />
                                            <span style={{ fontSize: '10px', color: '#aaa' }}>PEs</span>
                                        </div>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                                            <div ref={energyThRef} style={{ width: '100%', height: `${thermH}px`, background: '#ff375f' }} />
