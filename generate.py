...
def generic(subj, num):
    return [f"{subj} Chapter {i}" for i in range(1, num + 1)]
...
classes = {
    "Class 12": {
        ...
        "Accountancy": generic("Accountancy", 10),
        "History": generic("History", 15),
        "Geography": generic("Geography", 10),
        "Political Science": generic("Political Science", 9)
    },
    "Class 11": {
        ...
        "English": generic("English", 12),
        "Business Studies": generic("Business Studies", 10),
        "Economics": generic("Economics", 10),
        "Accountancy": generic("Accountancy", 10),
        "History": generic("History", 11),
        "Geography": generic("Geography", 12),
        "Political Science": generic("Political Science", 10)
    },
    "Class 10": {
        ...
        "English": generic("English", 15),
        "Social Science": generic("Social Science", 15)
    },
    "Class 9": {
    ...
    "Class 8": {
        ...
        "English": generic("English", 10),
        "Social Science": generic("Social Science", 12)
    },
    "Class 7": {
        ...
        "English": generic("English", 10),
        "Social Science": generic("Social Science", 10)
    },
    "Class 6": {
...
output = "export const chaptersData = {\n"
for c_name, subjects in classes.items():
    output += f'  "{c_name}": {{\n'
    for s_name, chapters in subjects.items():
        ...
        for i, ch in enumerate(chapters):
            ...
            output += f"      {{ id: 'c{i+1}', title: '{ch_escaped}' }}{comma}\n"
        comma2 = "," if list(subjects.keys()).index(s_name) < len(subjects) - 1 else ""
        output += f"    ]{comma2}\n"
    comma3 = "," if list(classes.keys()).index(c_name) < len(classes) - 1 else ""
    output += f"  }}{comma3}\n"
output += "};\n"

with open("src/chaptersData.js", "w") as f:
    f.write(output)
print("Done updating generator")
