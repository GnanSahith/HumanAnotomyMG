...
function CustomMassesandSpringsBasicsInner({
  ...
  useEffect(() => {
    gravityValueRef.current = gravityValue;
  }, [gravityValue]);
  useEffect(() => {
    dampingValueRef.current = dampingValue;
  }, [dampingValue]);
  useEffect(() => {
  ...
  const handleDampingChange = preset => {
    ...
    let val = 0.15;
    if (preset === 'none') val = 0.0;else if (preset === 'lots') val = 0.65;
    setDampingValue(val);
  };
  const handleGravityChange = preset => {
    setGravityPreset(preset);
    let val = 9.81;
    if (preset === 'moon') val = 1.62;else if (preset === 'jupiter') val = 24.79;
    setGravityValue(val);
  };
  const handleReset = () => {
    setIsPlaying(false);
  ...
  // JSX Render
  // ----------------------------------------------------
  return <div className="w-full h-full flex flex-col bg-transparent text-white font-sans" style={{
    paddingTop: '80px',
    height: '100%',
    overflowY: 'auto'
  }}>
            <div className="flex justify-end items-center px-6 py-4 border-b border-white/5 bg-white/[0.01] backdrop-blur-md sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    {/* Play/Pause control */}
                    <button onClick={() => setIsPlaying(!isPlaying)} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-semibold transition cursor-pointer shadow-lg ${isPlaying ? 'bg-rose-500/20 text-rose-300 border border-rose-500/40 hover:bg-rose-500/35' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 hover:bg-emerald-500/35'}`}>
                        {isPlaying ? <Pause size={16} className="fill-current" /> : <Play size={16} className="fill-current" />}
                        {isPlaying ? 'Pause Simulation' : 'Run Simulation'}
                    </button>

                    {/* Speed Selector */}
                    <div className="flex bg-white/5 rounded-full p-1 border border-white/10" style={{
          ...
          color: 'white'
                        ...
                        <button onClick={() => setSpeedMode('normal')} className={`px-4 py-1.5 rounded-full text-xs font-semibold transition cursor-pointer ${speedMode === 'normal' ? 'bg-[#00f0ff] text-black shadow-md' : 'text-white/60 hover:text-white'}`}>
                            Normal Speed
                        </button>
                        <button onClick={() => setSpeedMode('slow')} className={`px-4 py-1.5 rounded-full text-xs font-semibold transition cursor-pointer ${speedMode === 'slow' ? 'bg-[#00f0ff] text-black shadow-md' : 'text-white/60 hover:text-white'}`}>
                            Slow Motion
                        </button>
                    </div>

                    {/* Reset All */}
                    <button onClick={handleReset} className="flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-semibold bg-white/5 hover:bg-white/10 border border-white/10 transition cursor-pointer text-white/80 hover:text-white" style={{
          background: 'rgba(20, 20, 30, 0.8)',
          border: '1px solid rgba(255,255,255,0.1)',
          backdropFilter: 'blur(12px)',
          borderRadius: '16px',
          ...
          pointerEvents: 'auto'
                        ...
                        <div className="flex flex-col items-center h-full justify-end w-[16%] relative group">
                            ...
                            <div className="w-full bg-emerald-500 rounded-t shadow-[0_0_12px_rgba(16,185,129,0.3)] transition-all duration-75" ref={el => domRefs.current.keBar = el} style={{
              height: '0%'
            }}></div>
                        ...
                        <div className="flex flex-col items-center h-full justify-end w-[16%] relative group">
                            ...
                            <div className="w-full bg-[#00f0ff] rounded-t shadow-[0_0_12px_rgba(0,240,255,0.3)] transition-all duration-75" ref={el => domRefs.current.peElasBar = el} style={{
              height: '0%'
            }}></div>
                        ...
                        <div className="flex flex-col items-center h-full justify-end w-[16%] relative group">
                            ...
                            <div className="w-full bg-amber-500 rounded-t shadow-[0_0_12px_rgba(245,158,11,0.3)] transition-all duration-75" ref={el => domRefs.current.peGravBar = el} style={{
              height: '0%'
            }}></div>
                        ...
                        <div className="flex flex-col items-center h-full justify-end w-[16%] relative group">
                            ...
                            <div className="w-full bg-rose-500 rounded-t shadow-[0_0_12px_rgba(239,68,68,0.3)] transition-all duration-75" ref={el => domRefs.current.thermalBar = el} style={{
              height: '0%'
            }}></div>
                        ...
                        <div className="flex flex-col items-center h-full justify-end w-[18%] relative group border-l border-white/10 pl-2">
                            ...
                            <div className="w-full bg-yellow-400 rounded-t shadow-[0_0_15px_rgba(234,179,8,0.4)] transition-all duration-75" ref={el => domRefs.current.totalBar = el} style={{
              height: '0%'
            }}></div>
                    ...
                    <div className="bg-white/5 rounded-xl p-3 border border-white/5 flex flex-col gap-2.5" style={{
          background: 'rgba(20, 20, 30, 0.8)',
          border: '1px solid rgba(255,255,255,0.1)',
          backdropFilter: 'blur(12px)',
          borderRadius: '16px',
          color: 'white'
                ...
                <div className="flex flex-col gap-4 bg-white/[0.01] border border-white/5 rounded-2xl p-4 relative overflow-hidden items-center justify-center">
                    ...
                    {showStopwatch && <div className="absolute top-6 right-6 bg-[#181824]/90 backdrop-blur-md border border-white/15 rounded-xl p-3.5 flex flex-col items-center gap-2 shadow-2xl z-20 w-44">
                            ...
                            <div className="flex gap-2 w-full">
                                <button onClick={handleToggleStopwatch} className={`flex-1 text-[10px] font-bold py-1 px-2 rounded cursor-pointer transition ${stopwatchRunning ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30 hover:bg-rose-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 hover:bg-emerald-500/30'}`}>
                                    {stopwatchRunning ? 'Stop' : 'Start'}
                                </button>
                                <button onClick={handleResetStopwatch} className="flex-1 bg-white/5 text-white/70 hover:text-white border border-white/10 text-[10px] font-bold py-1 px-2 rounded cursor-pointer transition" style={{
              background: 'rgba(20, 20, 30, 0.8)',
              border: '1px solid rgba(255,255,255,0.1)',
              backdropFilter: 'blur(12px)',
              borderRadius: '16px',
          ...
          inset: 0
                        ...
                        <canvas ref={canvasRef} width={500} height={700} style={{
            width: '100%',
            height: '100%',
            maxHeight: '640px',
            background: '#0a0a0f',
            ...
            touchAction: 'none'
                ...
                <div className="flex flex-col gap-5 h-full overflow-y-auto pr-1">
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col gap-3 backdrop-blur-xl">
                        ...
                        <input type="range" min="1" max="10" step="0.5" value={springConstant} onChange={e => setSpringConstant(Number(e.target.value))} className="w-full accent-[#00f0ff] cursor-pointer" />
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col gap-3 backdrop-blur-xl">
                        ...
                        {/* Preset Buttons */}
                        <div className="grid grid-cols-3 gap-2 bg-white/5 p-1 rounded-lg border border-white/10" style={{
            background: 'rgba(20, 20, 30, 0.8)',
            border: '1px solid rgba(255,255,255,0.1)',
            backdropFilter: 'blur(12px)',
            borderRadius: '16px',
            color: 'white'
                            ...
                            {['none', 'friction', 'lots'].map(d => <button key={d} onClick={() => handleDampingChange(d)} className={`py-1 px-2 rounded capitalize text-xs font-semibold cursor-pointer transition ${dampingPreset === d ? 'bg-[#00f0ff] text-black shadow' : 'text-white/60 hover:text-white'}`}>
                                    {d === 'friction' ? 'Medium' : d}
                                </button>)}
                        </div>

                        {/* Custom Slider */}
                        <input type="range" min="0" max="1.2" step="0.05" value={dampingValue} onChange={e => {
            ...
            color: 'white'
                            ...
                            {['moon', 'earth', 'jupiter'].map(g => <button key={g} onClick={() => handleGravityChange(g)} className={`py-1 px-2 rounded capitalize text-xs font-semibold cursor-pointer transition ${gravityPreset === g ? 'bg-[#00f0ff] text-black shadow' : 'text-white/60 hover:text-white'}`}>
                                    {g}
                                </button>)}
                        </div>

                        {/* Custom slider */}
                        <input type="range" min="0" max="28" step="0.1" value={gravityValue} onChange={e => {
            ...
            setGravityPreset('custom');
                    ...
                    <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col gap-3.5 backdrop-blur-xl">
                        ...
                        <label className="flex items-center gap-3 text-xs text-white/75 cursor-pointer select-none">
                            <input type="checkbox" checked={showNaturalLength} onChange={e => setShowNaturalLength(e.target.checked)} className="w-4 h-4 accent-[#00f0ff] rounded border-white/10" />
                            Natural Length (Cyan line)
                        </label>

                        <label className="flex items-center gap-3 text-xs text-white/75 cursor-pointer select-none">
                            <input type="checkbox" checked={showEquilibrium} onChange={e => setShowEquilibrium(e.target.checked)} className="w-4 h-4 accent-[#00f0ff] rounded border-white/10" />
                            Equilibrium Position (Green line)
                        </label>

                        <label className="flex items-center gap-3 text-xs text-white/75 cursor-pointer select-none">
                            <input type="checkbox" checked={showRuler} onChange={e => setShowRuler(e.target.checked)} className="w-4 h-4 accent-[#00f0ff] rounded border-white/10" />
                            Mobile Measurement Ruler
                        </label>

                        <label className="flex items-center gap-3 text-xs text-white/75 cursor-pointer select-none">
                            <input type="checkbox" checked={showStopwatch} onChange={e => setShowStopwatch(e.target.checked)} className="w-4 h-4 accent-[#00f0ff] rounded border-white/10" />
                            Lab Stopwatch
                        </label>

                        <label className="flex items-center gap-3 text-xs text-white/75 cursor-pointer select-none">
                            <input type="checkbox" checked={showGraph} onChange={e => setShowGraph(e.target.checked)} className="w-4 h-4 accent-[#00f0ff] rounded border-white/10" />
                            Displacement Oscilloscope
                        </label>
                    </div>

                    {/* Oscilloscope Container */}
                    {showGraph && <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 flex flex-col gap-2 backdrop-blur-xl h-48">
                            <div className="w-full flex-1 rounded-lg overflow-hidden border border-white/15">
                                <canvas ref={graphCanvasRef} width={330} height={140} className="w-full h-full block" />
                            </div>
                        </div>}
                </div>
            </div>
            ...
            <div className="mx-6 mb-8 mt-4 bg-white/[0.02] border border-white/5 rounded-2xl p-5 backdrop-blur-xl">
                <div className="flex border-b border-white/10 mb-4 gap-2">
                    <button onClick={() => setActiveTab('theory')} className={`pb-2.5 px-4 font-semibold text-xs uppercase tracking-wider transition border-b-2 cursor-pointer ${activeTab === 'theory' ? 'border-[#00f0ff] text-[#00f0ff]' : 'border-transparent text-white/50 hover:text-white'}`}>
                        <span className="flex items-center gap-1.5"><BookOpen size={14} /> Physics Theory</span>
                    </button>
                    <button onClick={() => setActiveTab('energy')} className={`pb-2.5 px-4 font-semibold text-xs uppercase tracking-wider transition border-b-2 cursor-pointer ${activeTab === 'energy' ? 'border-[#00f0ff] text-[#00f0ff]' : 'border-transparent text-white/50 hover:text-white'}`}>
                        <span className="flex items-center gap-1.5"><HelpCircle size={14} /> Energy Conservation</span>
                    </button>
                    <button onClick={() => setActiveTab('activities')} className={`pb-2.5 px-4 font-semibold text-xs uppercase tracking-wider transition border-b-2 cursor-pointer ${activeTab === 'activities' ? 'border-[#00f0ff] text-[#00f0ff]' : 'border-transparent text-white/50 hover:text-white'}`}>
                        <span className="flex items-center gap-1.5"><Info size={14} /> Lab Exercises</span>
                    </button>
                </div>

                {activeTab === 'theory' && <div className="text-sm text-white/70 leading-relaxed flex flex-col gap-3">
                        <p>
                ...
                {activeTab === 'energy' && <div className="text-sm text-white/70 leading-relaxed flex flex-col gap-3">
                        <p>
                ...
                {activeTab === 'activities' && <div className="text-sm text-white/70 leading-relaxed flex flex-col gap-3">
...
export default function CustomMassesandSpringsBasics({
  onBack,
  title
}) {
  return <div style={{
