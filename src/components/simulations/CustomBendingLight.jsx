...
export default function CustomBendingLight({ onBack, title }) {
  // --- Simulation State ---
  ...
  // Movable coordinates
  const [protractorPos, setProtractorPos] = useState({ x: 400, y: 250 });
  const [protractorAngle, setProtractorAngle] = useState(0); // in radians
  const [intensityProbePos, setIntensityProbePos] = useState({ x: 220, y: 120 });
  const [speedProbePos, setSpeedProbePos] = useState({ x: 220, y: 380 });
  ...
  const resetSim = () => {
    ...
    setProtractorPos({ x: 400, y: 250 });
    setProtractorAngle(0);
    setIntensityProbePos({ x: 220, y: 120 });
    setSpeedProbePos({ x: 220, y: 380 });
    ...
    const render = (timestamp) => {
      ...
      if (!isTIR) {
        ...
        if (xY500 >= 0 && xY500 <= width) {
          L_refr = tY500;
          P_refr = { x: xY500, y: 500 };
        } else if (xY500 > width) {
          L_refr = (width - 400) / sinTheta2;
          P_refr = { x: width, y: 250 + L_refr * cosTheta2 };
        } else {
          L_refr = (0 - 400) / sinTheta2;
          P_refr = { x: 0, y: 250 + L_refr * cosTheta2 };
        }
      }
      ...
      if (intensityProbeActive) {
        ...
        ctx.bezierCurveTo(
          intensityProbePos.x + 20, intensityProbePos.y - 10,
          boxX - 20, boxY + 20,
          boxX, boxY + 15
        );
        ctx.stroke();
      ...
      if (speedProbeActive) {
        ...
        ctx.bezierCurveTo(
          speedProbePos.x + 20, speedProbePos.y - 10,
          sBoxX - 20, sBoxY + 20,
          sBoxX, sBoxY + 15
        );
        ctx.stroke();
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'absolute', left: '20px', top: '100px', display: 'flex', flexDirection: 'column', gap: '12px', zIndex: 10, pointerEvents: 'none' }}>
        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', padding: '12px 16px', borderRadius: '12px', color: 'white', width: '200px' }}>
          <span style={{ display: 'block', fontSize: '9px', color: '#888', fontWeight: 'bold', tracking: '0.05em', textTransform: 'uppercase' }}>Medium 1 (Upper)</span>
          <span style={{ fontSize: '15px', fontWeight: 'bold', display: 'block', margin: '2px 0' }}>{MATERIALS[medium1Type]?.name || "Custom"}</span>
          <span style={{ fontSize: '13px', fontFamily: 'monospace', color: '#3498db', fontWeight: '600' }}>n = {n1.toFixed(3)}</span>
        </div>

        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', padding: '12px 16px', borderRadius: '12px', color: 'white', width: '200px' }}>
          <span style={{ display: 'block', fontSize: '9px', color: '#888', fontWeight: 'bold', tracking: '0.05em', textTransform: 'uppercase' }}>Medium 2 (Lower)</span>
          <span style={{ fontSize: '15px', fontWeight: 'bold', display: 'block', margin: '2px 0' }}>{MATERIALS[medium2Type]?.name || "Custom"}</span>
          <span style={{ fontSize: '13px', fontFamily: 'monospace', color: '#2ecc71', fontWeight: '600' }}>n = {n2.toFixed(3)}</span>
        </div>
      </div>

      {/* Floating Instruction Tips */}
      <div style={{ position: 'absolute', left: '20px', bottom: '20px', display: 'flex', flexDirection: 'column', gap: '6px', zIndex: 10, pointerEvents: 'none', fontSize: '11px', color: '#888' }}>
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Sparkles size={12} style={{ color: '#ff375f' }} /> Click and drag the Laser Pointer to rotate</span>
        <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}><Info size={12} style={{ color: '#3498db' }} /> Drag tools from the panel onto the canvas</span>
      </div>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: '16px' }}>
          <button
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: '#aaa' }}>Laser Gun</span>
            </div>
            <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span style={{ color: '#ccc' }}>Wavelength</span>
              <span style={{ color: activeColor, fontWeight: 'bold', fontFamily: 'monospace' }}>{wavelength} nm</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <div
                style={{ width: '12px', height: '12px', borderRadius: '50%', backgroundColor: activeColor, border: '1px solid rgba(255,255,255,0.2)' }}
              />
              <input
                type="range"
                min="380"
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: '#aaa' }}>Media Configuration</span>
          </div>

          {/* Medium 1 select */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '11px', color: '#888' }}>UPPER MEDIUM (1)</label>
            <select
              value={medium1Type}
              onChange={(e) => setMedium1Type(e.target.value)}
              style={{
                width: '100%',
                background: 'rgba(255, 255, 255, 0.05)',
                border: '1px solid rgba(255, 255, 255, 0.1)',
            ...
            >
              <option value="air" style={{ background: '#14141e' }}>Air (n = 1.00)</option>
              <option value="water" style={{ background: '#14141e' }}>Water (n = 1.33)</option>
              <option value="glass" style={{ background: '#14141e' }}>Glass (n = 1.50)</option>
              <option value="custom" style={{ background: '#14141e' }}>Custom Index</option>
            </select>
            {medium1Type === 'custom' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#666', fontFamily: 'monospace' }}>
                  <span>n = 0.75</span>
                  <span style={{ color: '#3498db', fontWeight: 'bold' }}>n = {n1Custom.toFixed(2)}</span>
                  <span>n = 2.50</span>
                </div>
                <input
                  type="range"
                  ...
                  step="0.01"
                  value={n1Custom}
                  onChange={(e) => setN1Custom(parseFloat(e.target.value))}
          ...
          {/* Medium 2 select */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            <label style={{ fontSize: '11px', color: '#888' }}>LOWER MEDIUM (2)</label>
            <select
              value={medium2Type}
              onChange={(e) => setMedium2Type(e.target.value)}
              style={{
                width: '100%',
                background: 'rgba(255, 255, 255, 0.05)',
                border: '1px solid rgba(255, 255, 255, 0.1)',
            ...
            {medium2Type === 'custom' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginTop: '4px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#666', fontFamily: 'monospace' }}>
                  <span>n = 0.75</span>
                  <span style={{ color: '#3498db', fontWeight: 'bold' }}>n = {n2Custom.toFixed(2)}</span>
                  <span>n = 2.50</span>
                </div>
                <input
                  type="range"
                  ...
                  step="0.01"
                  value={n2Custom}
                  onChange={(e) => setN2Custom(parseFloat(e.target.value))}
                  style={{ accentColor: '#3498db', cursor: 'pointer', width: '100%' }}
                />
              </div>
            )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Activity size={16} style={{ color: '#bf5af2' }} />
            <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: '#aaa' }}>Visualization</span>
          </div>

          {/* Ray vs Wave Toggle */}
          <div style={{ display: 'flex', background: 'rgba(0, 0, 0, 0.2)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '10px', padding: '2px', gap: '4px' }}>
            <button
              onClick={() => setIsWaveModel(false)}
              style={{
                flex: 1,
                padding: '8px',
                borderRadius: '8px',
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '12px', color: '#ccc' }}>
            <input
              ...
              style={{ accentColor: '#3498db', cursor: 'pointer', width: '15px', height: '15px' }}
            />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Compass size={16} style={{ color: '#2ecc71' }} />
            <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: '#aaa' }}>Interactive Tools</span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            ...
            <button
              onClick={() => setIntensityProbeActive(!intensityProbeActive)}
              style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
            ...
            <button
              onClick={() => setSpeedProbeActive(!speedProbeActive)}
              style={{
                gridColumn: 'span 2',
                display: 'flex',
                alignItems: 'center',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', borderTop: '1px solid rgba(255, 255, 255, 0.05)', paddingTop: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Zap size={16} style={{ color: '#f1c40f' }} />
            <span style={{ fontSize: '12px', fontWeight: 'bold', textTransform: 'uppercase', color: '#aaa' }}>Time of Flight</span>
          </div>
          <button
            onClick={launchPulse}
            disabled={!laserOn}
