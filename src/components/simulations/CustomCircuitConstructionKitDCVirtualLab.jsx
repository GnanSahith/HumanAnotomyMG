...
import { ArrowLeft, RotateCcw, Sliders, Trash2, Play, Pause, Activity, Lightbulb, Battery, Eye, EyeOff, Scissors, Info, Settings2 } from 'lucide-react';
...
export default function CustomCircuitConstructionKitDCVirtualLab({
  ...
  const redProbeRef = useRef({
    x: 740,
    y: 120,
    isDragged: false,
    snappedTo: null
  });
  const blackProbeRef = useRef({
    x: 670,
    y: 120,
    isDragged: false,
    snappedTo: null
  ...
  useEffect(() => {
    selectedJunctionRef.current = selectedJunction;
  }, [selectedJunction]);
  useEffect(() => {
    representationRef.current = representation;
  }, [representation]);
  useEffect(() => {
    showElectronsRef.current = showElectrons;
  }, [showElectrons]);
  useEffect(() => {
    showValuesRef.current = showValues;
  }, [showValues]);
  useEffect(() => {
    voltmeterEnabledRef.current = voltmeterEnabled;
  }, [voltmeterEnabled]);
  useEffect(() => {
    ammeterEnabledRef.current = ammeterEnabled;
  }, [ammeterEnabled]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      comps.forEach((c, idx) => {
        ...
        if (reps === 'lifelike') {
          ...
          if (c.type === 'wire') {
            ...
            const stripeW = 3.5;
            const stripePositions = [bodyStart + bodyL * 0.2, bodyStart + bodyL * 0.4, bodyStart + bodyL * 0.6, bodyStart + bodyL * 0.8];
            ...
            ctx.lineWidth = 1.5;
            ctx.beginPath();
            ctx.moveTo(bulbCenter - 8, -3);
            ctx.lineTo(bulbCenter + 8, -3);
            ctx.moveTo(bulbCenter - 8, 1);
            ctx.lineTo(bulbCenter + 8, 1);
            ctx.stroke();
      ...
      if (voltOn) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(660, 45, 100, 50, 6);else ctx.rect(660, 45, 100, 50);
        ctx.fill();
        ctx.stroke();
      ...
      if (ammOn) {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        if (ctx.roundRect) ctx.roundRect(660, 265, 100, 50, 6);else ctx.rect(660, 265, 100, 50);
        ctx.fill();
        ctx.stroke();
        ...
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(ammP.x - 18, ammP.y);
        ctx.lineTo(ammP.x + 18, ammP.y);
        ctx.moveTo(ammP.x, ammP.y - 18);
        ctx.lineTo(ammP.x, ammP.y + 18);
        ctx.stroke();
        if (snapped) {
  ...
  const handleAddComponent = type => {
    ...
    const newComp = {
      ...
      x2: spawnX1 + 120,
      y2: spawnY1,
      value: type === 'battery' ? 9 : 10,
      // Default 9V battery, 10 ohm resistor/bulb
  ...
  const handleSplitJunction = () => {
    ...
    const matching = [];
    components.forEach(c => {
      if (Math.abs(c.x1 - x) < 3 && Math.abs(c.y1 - y) < 3) matching.push({
        id: c.id,
        termIndex: 1
      });
      if (Math.abs(c.x2 - x) < 3 && Math.abs(c.y2 - y) < 3) matching.push({
        id: c.id,
        termIndex: 2
      });
    });
      ...
      pointerEvents: 'auto'
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            type: 'wire',
            label: 'Wire',
            color: 'bg-yellow-500/10 border-yellow-500/30 hover:bg-yellow-500/20 text-yellow-300',
            icon: MinusIcon
          }, {
            type: 'battery',
            label: 'Battery',
            color: 'bg-emerald-500/10 border-emerald-500/30 hover:bg-emerald-500/20 text-emerald-300',
            icon: Battery
          }, {
            type: 'resistor',
            label: 'Resistor',
            color: 'bg-orange-500/10 border-orange-500/30 hover:bg-orange-500/20 text-orange-300',
            icon: Activity
          }, {
            type: 'lightbulb',
            label: 'Light Bulb',
            color: 'bg-cyan-500/10 border-cyan-500/30 hover:bg-cyan-500/20 text-cyan-300',
            icon: Lightbulb
          }, {
            type: 'switch',
            label: 'Switch',
            color: 'bg-indigo-500/10 border-indigo-500/30 hover:bg-indigo-500/20 text-indigo-300',
            icon: Sliders
          }].map(item => <button key={item.type} onClick={() => handleAddComponent(item.type)} className="flex flex-col items-center gap-2 p-3.5 transition-all text-center font-medium" style={{
            ...
            e.currentTarget.style.borderColor = 'rgba(255, 255, 255, 0.1)';
              ...
              color: item.color.includes('yellow') ? '#f1c40f' : item.color.includes('emerald') ? '#2ecc71' : item.color.includes('orange') ? '#e67e22' : item.color.includes('cyan') ? '#00bcd4' : '#9b59b6'
            }} />
          ...
          <div className="mt-8 p-4 rounded-xl border border-slate-800/80" style={{
          background: 'rgba(20, 20, 30, 0.8)',
          border: '1px solid rgba(255,255,255,0.1)',
          backdropFilter: 'blur(12px)',
          borderRadius: '16px',
      ...
      pointerEvents: 'auto'
        ...
        <div className="space-y-6">
          ...
          <div className="flex border-b border-slate-800/60 pb-1.5 gap-4">
            {['inspector', 'tools', 'settings'].map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`text-xs font-bold uppercase tracking-wider pb-1 transition-all ${activeTab === tab ? 'text-[#3498db] border-b border-[#3498db]' : 'text-slate-500 hover:text-slate-300'}`}>
                {tab}
              </button>)}
          </div>
          
          {/* Tab Contents */}
          {activeTab === 'inspector' && <div className="space-y-4">
              {selectedComponent ? <div className="space-y-4 p-4 rounded-xl border border-slate-800/80" style={{
            background: 'rgba(20, 20, 30, 0.8)',
            border: '1px solid rgba(255,255,255,0.1)',
            backdropFilter: 'blur(12px)',
            borderRadius: '16px',
            color: 'white'
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => handleDeleteComponent(selectedComponent.id)} className="text-red-400 hover:text-red-300 p-1 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-lg transition-all" title="Delete Element">
                      <Trash2 size={15} />
                    </button>
                  </div>
                  
                  {/* Variable adjustment slider */}
                  {(selectedComponent.type === 'battery' || selectedComponent.type === 'resistor' || selectedComponent.type === 'lightbulb') && <div className="space-y-2">
                      ...
                      <input type="range" min={selectedComponent.type === 'battery' ? 0 : 0.5} max={selectedComponent.type === 'battery' ? 120 : 200} step={selectedComponent.type === 'battery' ? 1 : 0.5} value={selectedComponent.value} onChange={e => handleUpdateComponentValue(selectedComponent.id, parseFloat(e.target.value))} className="w-full h-1.5 rounded-lg appearance-none cursor-pointer ds-slider" style={{
                background: 'rgba(20, 20, 30, 0.8)',
                border: '1px solid rgba(255,255,255,0.1)',
                backdropFilter: 'blur(12px)',
                borderRadius: '16px',
                color: 'white'
                  ...
                  {selectedComponent.type === 'switch' && <div className="flex justify-between items-center py-1.5">
                ...
                </div> : selectedJunction ? <div className="space-y-4 p-4 rounded-xl border border-slate-800/80" style={{
            background: 'rgba(20, 20, 30, 0.8)',
            border: '1px solid rgba(255,255,255,0.1)',
            backdropFilter: 'blur(12px)',
            borderRadius: '16px',
            color: 'white'
                  ...
                  <button onClick={handleSplitJunction} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-yellow-500/10 border border-yellow-500/30 hover:bg-yellow-500/20 text-yellow-300 font-semibold text-xs rounded-xl transition-all">
                    <Scissors size={14} />
                    Split Junction Wires
                  </button>
                </div> : <div className="text-xs text-slate-500 text-center py-8">
          ...
          {activeTab === 'tools' && <div className="space-y-4">
              <div className="p-4 rounded-xl border border-slate-800/80 space-y-3" style={{
            background: 'rgba(20, 20, 30, 0.8)',
            border: '1px solid rgba(255,255,255,0.1)',
            backdropFilter: 'blur(12px)',
            borderRadius: '16px',
            color: 'white'
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setVoltmeterEnabled(!voltmeterEnabled)} className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${voltmeterEnabled ? 'bg-[#3498db]' : ''}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setAmmeterEnabled(!ammeterEnabled)} className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${ammeterEnabled ? 'bg-[#3498db]' : ''}`}>
              ...
              <div className="space-y-3">
                {voltmeterEnabled && <div className="p-3 rounded-lg border border-slate-850 flex justify-between items-center text-xs" style={{
              background: 'rgba(20, 20, 30, 0.8)',
              border: '1px solid rgba(255,255,255,0.1)',
              backdropFilter: 'blur(12px)',
              borderRadius: '16px',
              color: 'white'
                ...
                {ammeterEnabled && <div className="p-3 rounded-lg border border-slate-850 flex justify-between items-center text-xs" style={{
              background: 'rgba(20, 20, 30, 0.8)',
              border: '1px solid rgba(255,255,255,0.1)',
              backdropFilter: 'blur(12px)',
              borderRadius: '16px',
          ...
          {activeTab === 'settings' && <div className="space-y-4">
              <div className="p-4 rounded-xl border border-slate-800/80 space-y-4" style={{
            background: 'rgba(20, 20, 30, 0.8)',
            border: '1px solid rgba(255,255,255,0.1)',
            backdropFilter: 'blur(12px)',
            borderRadius: '16px',
            color: 'white'
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowElectrons(!showElectrons)} className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${showElectrons ? 'bg-[#3498db]' : ''}`}>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setShowValues(!showValues)} className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${showValues ? 'bg-[#3498db]' : ''}`}>
