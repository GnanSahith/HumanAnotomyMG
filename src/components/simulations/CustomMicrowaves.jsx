...
const CustomMicrowavesInner = ({ onBack, title }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', paddingRight: '320px' }}>
        <div style={{ border: '1px solid rgba(255,255,255,0.1)', borderRadius: '16px', overflow: 'hidden', background: 'rgba(20,20,30,0.4)', backdropFilter: 'blur(8px)', boxShadow: '0 8px 32px rgba(0,0,0,0.4)', position: 'relative' }}>
          <canvas
            ref={canvasRef}
            width={800}
            height={400}
            style={{ display: 'block' }}
          />
          <div style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.1)', padding: '6px 12px', borderRadius: '8px', fontSize: '12px', color: '#ccc', fontFamily: 'monospace' }}>
            Time: <span style={{ color: '#3498db', fontWeight: 'bold' }}>{simState.current.time.toFixed(1)}s</span>
          </div>
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '1px' }}>Wave Generator</label>
          
          {/* Amplitude */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
              <span>Amplitude</span>
              <span style={{ color: '#3498db', fontWeight: 'bold' }}>{amplitude}%</span>
            </div>
            <input 
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
              <span>Frequency</span>
              <span style={{ color: '#9b59b6', fontWeight: 'bold' }}>{frequency} Hz</span>
            </div>
            <input 
        ...
        {/* Environment */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '1px' }}>Environment</label>
          
          {/* Molecule Count */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '14px' }}>
              <span>Water Molecules</span>
              <span style={{ color: '#2ecc71', fontWeight: 'bold' }}>{moleculeCount}</span>
            </div>
            <input 
              type="range" min="1" max="50" 
              value={moleculeCount} onChange={(e) => setMoleculeCount(Number(e.target.value))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <label style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '1px' }}>Thermal Physics</label>
...
export default function CustomMicrowaves({ onBack, title }) {
