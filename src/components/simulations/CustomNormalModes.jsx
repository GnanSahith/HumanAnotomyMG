...
import { Play, Pause, RotateCcw, Activity, Sliders, Waves, Zap, ArrowLeft, ChevronRight, Eye, EyeOff, Info, Compass, GitMerge, ActivitySquare, Settings2 } from 'lucide-react';

// Color map for distinct normal modes (Mode 1 to Mode 10+)
const MODE_COLORS = ['#ff3b30',
// Mode 1: Red
'#ff9500',
// Mode 2: Orange
'#ffcc00',
...
function CustomNormalModesInner({
  onBack,
  title
}) {
  ...
  const drawSpring = (ctx, x1, y1, x2, y2, coils = 12, width = 6) => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      if (i > 0 && i < steps) {
        const phase = i % 4;
        if (phase === 1) offset = width;else if (phase === 3) offset = -width;
      }
    ...
    color: '#fff'
      ...
      justifyContent: 'center'
            ...
            <div className="relative w-full max-w-[850px] aspect-[850/380] flex items-center justify-center p-2">
              <canvas ref={canvasRef} width={850} height={380} className="w-full h-auto max-w-full drop-shadow-[0_0_25px_rgba(168,85,247,0.15)] cursor-ns-resize" onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} />
              {draggedMass === null && <div style={{
          position: 'absolute',
          top: '10px',
          left: '50%',
        ...
        backdropFilter: 'blur(12px)'
          ...
          alignItems: 'center',
          gap: '6px',
          background: playing ? 'rgba(241, 196, 15, 0.2)' : 'rgba(46, 204, 113, 0.2)',
          border: playing ? '1px solid #f1c40f' : '1px solid #2ecc71',
          color: playing ? '#f1c40f' : '#2ecc71',
          padding: '6px 16px',
          borderRadius: '12px',
          cursor: 'pointer',
          fontWeight: 600,
          fontSize: '13px'
              ...
              {!playing && <button onClick={stepForward} style={{
          display: 'flex',
          alignItems: 'center',
          gap: '6px',
      ...
      gap: '20px'
        ...
        gap: '8px'
          ...
          fontSize: '12px'
              ...
              const period = 1 / hz;
              return <tr key={m} style={{
                borderBottom: '1px solid rgba(255,255,255,0.05)'
              }}>
                            <td style={{
                  padding: '4px 0',
                  fontWeight: 'bold',
                  color: MODE_COLORS[m % MODE_COLORS.length]
                }}>Mode {m + 1}</td>
                            <td style={{
                  padding: '4px 0',
                  textAlign: 'right',
      ...
      gap: '20px'
        ...
        gap: '8px'
          ...
          gap: '6px'
            ...
            id: 'clear',
            label: 'Clear'
          }].map(preset => <button key={preset.id} onClick={() => applyPreset(preset.id)} style={{
        ...
        gap: '8px'
          ...
          alignItems: 'center'
            ...
            fontWeight: 700
              ...
              <input type="range" min="1" max={MAX_MASSES} step="1" value={numMasses} onChange={e => setNumMasses(parseInt(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db'
        }} />
            </div>
        ...
        gap: '8px'
          ...
          padding: '4px'
                ...
                {['none', 'low', 'medium', 'high'].map(d => <button key={d} onClick={() => setDamping(d)} style={{
            flex: 1,
            padding: '6px 0',
            border: 'none',
            borderRadius: '6px',
        ...
        gap: '8px'
          ...
          padding: '4px'
                ...
                {['low', 'medium', 'high'].map(t => <button key={t} onClick={() => setTension(t)} style={{
            flex: 1,
            padding: '6px 0',
            border: 'none',
            borderRadius: '6px',
        ...
        gap: '8px'
          ...
          gap: '6px'
            ...
            fontSize: '12px'
                  ...
                  <input type="checkbox" checked={showIndividual} onChange={e => setShowIndividual(e.target.checked)} style={{
              accentColor: '#2ecc71'
            }} />
                </label>
                <label style={{
            ...
            fontSize: '12px'
                  ...
                  <input type="checkbox" checked={showSprings} onChange={e => setShowSprings(e.target.checked)} style={{
              accentColor: '#2ecc71'
            }} />
                </label>
                <label style={{
            ...
            fontSize: '12px'
                  ...
                  <input type="checkbox" checked={showFrequencies} onChange={e => setShowFrequencies(e.target.checked)} style={{
              accentColor: '#2ecc71'
            }} />
                </label>
                <label style={{
            ...
            fontSize: '12px'
                  ...
                  <input type="checkbox" checked={showPhaseWheels} onChange={e => setShowPhaseWheels(e.target.checked)} style={{
              accentColor: '#2ecc71'
            }} />
                </label>
              </div>
            ...
            borderRadius: '8px'
              ...
              fontSize: '12px'
                ...
                fontWeight: 'bold'
                      ...
                      <span style={{
                fontFamily: 'monospace'
              }}>Amp: {modeAmplitudes[m].toFixed(2)}</span>
                    </div>
                    <input type="range" min="-1" max="1" step="0.01" value={modeAmplitudes[m]} onChange={e => handleAmplitudeChange(m, e.target.value)} style={{
              width: '100%',
              accentColor: MODE_COLORS[m % MODE_COLORS.length]
            }} />
                    <div style={{
              display: 'flex',
              justifyContent: 'space-between',
              ...
              marginTop: '2px'
                    ...
                    <input type="range" min="0" max={Math.PI * 2} step="0.01" value={modePhases[m]} onChange={e => handlePhaseChange(m, e.target.value)} style={{
              width: '100%',
              accentColor: '#9b59b6'
            }} />
                  </div>)}
