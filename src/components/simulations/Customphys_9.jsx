...
const Customphys_9Inner = () => {
  ...
  const initSimulation = useCallback(() => {
    ...
    const ground = Matter.Bodies.rectangle(400, 390, 810, 20, { isStatic: true, render: { fillStyle: '#1e293b' } });
    const top = Matter.Bodies.rectangle(400, 10, 810, 20, { isStatic: true, render: { fillStyle: '#1e293b' } });
    const leftWall = Matter.Bodies.rectangle(10, 200, 20, 400, { isStatic: true, render: { fillStyle: '#1e293b' } });
    const rightWall = Matter.Bodies.rectangle(790, 200, 20, 400, { isStatic: true, render: { fillStyle: '#1e293b' } });
    ...
    Matter.Body.setVelocity(body1, { x: vel1, y: 0 });
    Matter.Body.setVelocity(body2, { x: vel2, y: 0 });
    ...
    Matter.Events.on(engine, 'beforeUpdate', () => {
      Matter.Body.setPosition(body1, { x: body1.position.x, y: 200 });
      Matter.Body.setPosition(body2, { x: body2.position.x, y: 200 });
      Matter.Body.setVelocity(body1, { x: body1.velocity.x, y: 0 });
      Matter.Body.setVelocity(body2, { x: body2.velocity.x, y: 0 });
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div ref={sceneRef} style={{ borderRadius: '16px', overflow: 'hidden' }} />
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          <span style={{ fontSize: '11px', color: '#ff4444', fontWeight: '600', letterSpacing: '0.05em' }}>OBJECT 1 (RED)</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Velocity:</span>
            <span style={{ fontFamily: 'monospace' }}>{currentV1.toFixed(2)} m/s</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Momentum:</span>
            <span style={{ fontFamily: 'monospace' }}>{currentP1.toFixed(2)} kg·m/s</span>
          </div>
        </div>

        {/* Object 2 Stats */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
          <span style={{ fontSize: '11px', color: '#3b82f6', fontWeight: '600', letterSpacing: '0.05em' }}>OBJECT 2 (BLUE)</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Velocity:</span>
            <span style={{ fontFamily: 'monospace' }}>{currentV2.toFixed(2)} m/s</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Momentum:</span>
            <span style={{ fontFamily: 'monospace' }}>{currentP2.toFixed(2)} kg·m/s</span>
          </div>
        </div>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '11px', color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em' }}>System Totals</span>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Momentum:</span>
            <span style={{ fontFamily: 'monospace', fontWeight: '600', color: '#a855f7' }}>{totalMomentum.toFixed(2)} kg·m/s</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span>Kinetic Energy:</span>
            <span style={{ fontFamily: 'monospace', fontWeight: '600', color: '#10b981' }}>{totalKinetic.toFixed(2)} J</span>
          </div>
        </div>
      </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <span style={{ fontSize: '11px', color: '#ff4444', fontWeight: '600', letterSpacing: '0.05em' }}>OBJECT 1 CONFIG</span>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span>Mass:</span>
              <span style={{ fontWeight: '600' }}>{mass1} kg</span>
            </div>
            <input 
              type="range" min="0.5" max="5" step="0.5" value={mass1} 
              onChange={(e) => setMass1(Number(e.target.value))}
              disabled={isPlaying}
              style={{ accentColor: '#ff4444', cursor: isPlaying ? 'not-allowed' : 'pointer' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span>Velocity:</span>
              <span style={{ fontWeight: '600' }}>{vel1} m/s</span>
            </div>
            <input 
              type="range" min="-10" max="10" step="1" value={vel1} 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
          <span style={{ fontSize: '11px', color: '#3b82f6', fontWeight: '600', letterSpacing: '0.05em' }}>OBJECT 2 CONFIG</span>
          
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span>Mass:</span>
              <span style={{ fontWeight: '600' }}>{mass2} kg</span>
            </div>
            <input 
              type="range" min="0.5" max="5" step="0.5" value={mass2} 
              onChange={(e) => setMass2(Number(e.target.value))}
              disabled={isPlaying}
              style={{ accentColor: '#3b82f6', cursor: isPlaying ? 'not-allowed' : 'pointer' }}
            />
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
              <span>Velocity:</span>
              <span style={{ fontWeight: '600' }}>{vel2} m/s</span>
            </div>
            <input 
              type="range" min="-10" max="10" step="1" value={vel2} 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', borderTop: '1px solid rgba(255,255,255,0.05)', paddingTop: '10px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px' }}>
            <span style={{ color: '#cbd5e1' }}>Elasticity (Restitution):</span>
            <span style={{ fontWeight: '600' }}>{elasticity}</span>
          </div>
          <input 
            type="range" min="0" max="1" step="0.1" value={elasticity} 
            onChange={(e) => setElasticity(Number(e.target.value))}
            disabled={isPlaying}
            style={{ accentColor: '#a855f7', cursor: isPlaying ? 'not-allowed' : 'pointer' }}
          />
        </div>
...
export default function Customphys_9({ onBack, title }) {
    return (
