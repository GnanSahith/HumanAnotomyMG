...
function CustomWaveOnAStringInner({
  ...
  const updatePhysics = () => {
    ...
    for (let i = 1; i < numPoints - 1; i++) {
      yNext[i] = 2 * y[i] - yPrev[i] + c2 * (y[i + 1] - 2 * y[i] + y[i - 1]) - damp * (y[i] - yPrev[i]);
    }
  ...
  return <div style={{
    ...
    color: '#fff'
      ...
      justifyContent: 'center'
                ...
                <svg viewBox="-100 -300 900 600" preserveAspectRatio="xMidYMid meet" style={{
        width: '100%',
        height: '100%',
        maxWidth: '800px',
        maxHeight: '600px',
        ...
        userSelect: 'none'
                    ...
                    {/* The String */}
                    <path d={`M 0 ${points[0]} ` + points.map((y, i) => `L ${i * dx} ${y}`).join(' ')} fill="none" stroke="#e74c3c" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" />
                    
                    {/* Beads */}
                    {points.map((y, i) => <circle key={i} cx={i * dx} cy={y} r={i === 0 ? 8 : 4} fill={i === 0 ? "#3498db" : i % 10 === 0 ? "#2ecc71" : "#e74c3c"} />)}

                    {/* Right Boundary Visuals */}
                    {endType === 'fixed' && <g transform={`translate(${(numPoints - 1) * dx}, 0)`}>
                            <rect x="-5" y="-100" width="10" height="200" fill="#8d6e63" />
                            <circle cx="0" cy="0" r="8" fill="#333" />
                        </g>}
                    {endType === 'loose' && <g transform={`translate(${(numPoints - 1) * dx}, 0)`}>
                    ...
                    <g transform={`translate(0, ${points[0]})`}>
                        ...
                        {mode === 'oscillate' && <g transform="translate(-30, 0)">
                        ...
                        {mode === 'pulse' && <g transform="translate(-30, 0)">
      ...
      gap: '20px'
        ...
        gap: '8px'
          ...
          padding: '4px'
                        ...
                        {['manual', 'oscillate', 'pulse'].map(m => <button key={m} onClick={() => setMode(m)} style={{
            flex: 1,
            padding: '8px 0',
            border: 'none',
            borderRadius: '6px',
        ...
        gap: '8px'
          ...
          justifyContent: 'space-between'
            ...
            fontWeight: 500
                        ...
                        <input type="range" min="-100" max="100" step="1" value={-manualY} onChange={e => setManualY(-Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#f1c40f'
        }} />
                    </div>}
                
                {mode === 'pulse' && <button onClick={triggerPulse} style={{
        ...
        gap: '8px'
          ...
          padding: '4px'
                        ...
                        {['fixed', 'loose', 'none'].map(e => <button key={e} onClick={() => setEndType(e)} style={{
            flex: 1,
            padding: '8px 0',
            border: 'none',
            borderRadius: '6px',
        ...
        gap: '8px'
          ...
          justifyContent: 'space-between'
            ...
            color: '#2ecc71',
            fontWeight: 700
          }}>{amplitude.toFixed(2)} cm</span>
                    </div>
                    <input type="range" min="0" max="1.25" step="0.01" value={amplitude} onChange={e => setAmplitude(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#2ecc71'
        }} />
                </div>
        ...
        gap: '8px'
          ...
          justifyContent: 'space-between'
            ...
            color: '#3498db',
            fontWeight: 700
          }}>{frequency.toFixed(2)} Hz</span>
                        </div>
                        <input type="range" min="0" max="3" step="0.01" value={frequency} onChange={e => setFrequency(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db'
        }} />
                    </div>}
        ...
        gap: '8px'
          ...
          justifyContent: 'space-between'
            ...
            color: '#9b59b6',
            fontWeight: 700
          }}>{damping === 0 ? 'None' : damping === 0.5 ? 'Lots' : damping.toFixed(2)}</span>
                    </div>
                    <input type="range" min="0" max="0.5" step="0.01" value={damping} onChange={e => setDamping(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#9b59b6'
        }} />
                    <div style={{
        ...
        gap: '8px'
          ...
          justifyContent: 'space-between'
            ...
            color: '#e74c3c',
            fontWeight: 700
          }}>{tension < 0.4 ? 'Low' : tension > 0.7 ? 'High' : 'Medium'}</span>
                    </div>
                    <input type="range" min="0.1" max="0.9" step="0.1" value={tension} onChange={e => setTension(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#e74c3c'
        }} />
                    <div style={{
...
export default function CustomWaveOnAString({
  onBack,
  title
}) {
