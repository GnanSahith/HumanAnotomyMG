...
import { Atom, Sun, Target, Activity, Settings, Maximize2, RotateCcw } from 'lucide-react';
...
function CustomModelsOfHydrogenAtomInner() {
  ...
  const state = useRef({
    photons: [], // incoming and outgoing photons
    electronLevel: 1, // 1 to 6
    electronAngle: 0,
    solarRadius: 60,
    time: 0,
                        ...
                        if (Math.abs(E_photon - E_diff) < 0.5) {
                            ...
                            setTimeout(() => {
                                if (state.current.electronLevel > 1) {
                                    ...
                                    state.current.photons.push({
                                        x: cx, y: cy,
                                        vx: 5 * Math.cos(angle), vy: 5 * Math.sin(angle),
                                        wl: emittedWl,
  ...
  return (
    <div style={{ display: 'flex', width: '100%', height: '100%', padding: '90px 20px 20px 20px', gap: '20px', boxSizing: 'border-box', overflow: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      {/* Left Column: Simulation Canvas & Spectrometer */}
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
        
        {/* Main Canvas Area */}
        <div style={{ position: 'relative', background: 'rgba(20, 20, 30, 0.6)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '450px', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: '15px', left: '15px', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 10 }}>
              <Sun className="h-5 w-5 text-yellow-400" />
              <span style={{ fontSize: '13px', fontWeight: 500, color: '#e2e8f0' }}>Light Source: {lightType}</span>
          </div>
          <div style={{ position: 'absolute', bottom: '15px', right: '15px', display: 'flex', alignItems: 'center', gap: '8px', zIndex: 10 }}>
              <span style={{ fontSize: '13px', fontWeight: 500, color: '#e2e8f0' }}>Model: {model}</span>
              <Atom className="h-5 w-5 text-purple-400" />
          </div>
          
          <div style={{ position: 'absolute', left: '15px', top: '50%', transform: 'translateY(-50%)', width: '48px', height: '96px', borderRadius: '0 24px 24px 0', border: '1px solid rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(20,20,30,0.8)' }}>
              <div style={{ width: '32px', height: '32px', borderRadius: '50%', boxShadow: '0 0 15px rgba(255,255,255,0.8)', backgroundColor: lightType === 'White' ? '#fff' : wavelengthToColor(wavelength) }}></div>
          </div>
          ...
          <canvas
            ...
            width={600}
            height={400}
            style={{ maxWidth: '100%', height: 'auto', filter: 'drop-shadow(0 0 15px rgba(168,85,247,0.2))' }}
          />
        </div>

        {/* Spectrometer */}
        <div style={{ background: 'rgba(20, 20, 30, 0.6)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', padding: '20px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '15px' }}>
              <Activity className="h-5 w-5 text-cyan-400" />
              <h2 style={{ fontSize: '16px', fontWeight: 600, color: 'white', margin: 0 }}>Spectrometer</h2>
           </div>
           <div style={{ width: '100%', height: '96px', borderRadius: '12px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.2)', position: 'relative' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', display: 'flex', justifyContent: 'space-between', padding: '4px 8px', fontSize: '10px', color: '#94a3b8', marginTop: '4px', zIndex: 10 }}>
                  <span>90nm (UV)</span>
                  <span>400nm</span>
                  <span>700nm (IR)</span>
              </div>
              <canvas ref={spectrometerRef} width={800} height={100} style={{ width: '100%', height: '100%' }} />
           </div>
        </div>
      ...
      <div style={{ width: '320px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
        
        {/* Playback Controls Card */}
        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <span style={{ fontSize: '12px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.8px', color: 'rgba(255,255,255,0.5)' }}>Simulation State</span>
          <div style={{ display: 'flex', gap: '10px' }}>
            <button
              onClick={() => setRunning(!running)}
              style={{ flex: 1, padding: '10px', background: running ? '#e74c3c' : '#2ecc71', border: 'none', borderRadius: '8px', color: 'white', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
            >
              {running ? 'Pause' : 'Play'}
            </button>
            <button
              ...
              style={{ flex: 1, padding: '10px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', color: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontWeight: 600, transition: 'all 0.2s' }}
            >
        ...
        {/* Model Selection */}
        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(12px)', padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <h2 style={{ fontSize: '15px', fontWeight: 600, color: 'white', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
             <Target className="h-5 w-5 text-rose-400" /> Model Selection
          </h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
             {['Billiard Ball', 'Plum Pudding', 'Classical Solar System', 'Bohr', 'de Broglie', 'Schrödinger'].map(m => (
                 <button 
                    key={m}
                    onClick={() => setModel(m)}
                    style={{
        ...
        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(12px)', padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
              <button 
                ...
                style={{ flex: 1, padding: '8px', fontSize: '12px', borderRadius: '8px', border: 'none', background: lightType === 'White' ? '#fff' : 'rgba(255,255,255,0.05)', color: lightType === 'White' ? '#0f172a' : '#94a3b8', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
              >
                  White Light
              </button>
              <button 
                onClick={() => setLightType('Monochromatic')}
                style={{ flex: 1, padding: '8px', fontSize: '12px', borderRadius: '8px', border: 'none', background: lightType === 'Monochromatic' ? '#a855f7' : 'rgba(255,255,255,0.05)', color: 'white', fontWeight: 'bold', cursor: 'pointer', transition: 'all 0.2s' }}
              >
                  Monochromatic
              </button>
          </div>
          ...
          {lightType === 'Monochromatic' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                      <span style={{ color: '#94a3b8' }}>Wavelength</span>
                      <span style={{ color: 'white', fontWeight: 'bold', fontFamily: 'monospace' }}>{wavelength} nm</span>
                  </div>
                  <input 
                      type="range"
                      min="90" max="700" step="1"
                      value={wavelength}
        ...
        <div style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255, 255, 255, 0.1)', backdropFilter: 'blur(12px)', padding: '20px', borderRadius: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
           ...
           <div style={{ height: '140px', borderLeft: '1px solid #475569', paddingLeft: '12px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', fontSize: '11px', fontFamily: 'monospace', color: '#94a3b8', position: 'relative' }}>
               ...
               <div style={{ color: 'white', fontWeight: 'bold' }}><span>n=1 (-13.6 eV) Ground</span></div>
           </div>
        </div>
...
export default function CustomModelsOfHydrogenAtom({ onBack, title }) {
    return (
