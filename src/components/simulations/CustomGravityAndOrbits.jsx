...
import { ArrowLeft, RotateCcw, Play, Pause, FastForward, Settings2 } from 'lucide-react';
const SCENARIOS = {
  SUN_EARTH: 'sun_earth',
...
const CustomGravityAndOrbits = ({
  ...
  const stateRef = useRef({
    ...JSON.parse(JSON.stringify(INITIAL_STATES[SCENARIOS.SUN_EARTH]))
  });
  ...
  const updatePhysics = () => {
    ...
    for (let step = 0; step < substeps; step++) {
      ...
      const accelerations = bodies.map(() => ({
        ax: 0,
        ay: 0
      }));
      if (gravityEnabled) {
        for (let i = 0; i < bodies.length; i++) {
          for (let j = i + 1; j < bodies.length; j++) {
            ...
            if (dist > (b1.radius + b2.radius) * 0.1) {
              // Avoid infinite force at singularity
    ...
    bodies.forEach(b => {
      if (!b.fixed && showPath) {
        if (b.path.length === 0 || Math.abs(b.path[b.path.length - 1].x - b.x) > 2 || Math.abs(b.path[b.path.length - 1].y - b.y) > 2) {
          b.path.push({
            x: b.x,
            y: b.y
          });
