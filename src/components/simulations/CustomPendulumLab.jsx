...
export default function CustomPendulumLab({
  ...
  // Visual State
  const [bobPos, setBobPos] = useState({
    x: 0,
    y: 0
  });
    ...
    overflow: 'hidden'
      ...
      zIndex: 10
        ...
        justifyContent: 'flex-start'
                    ...
                    {onBack && <button onClick={onBack} className="btn-back-hover" style={backButtonStyle}>
        ...
        alignItems: 'center'
                    ...
                    <button onClick={() => setIsPlaying(!isPlaying)} className="btn-action-hover" style={actionButtonStyle}>
                        {isPlaying ? <Pause size={18} /> : <Play size={18} />}
                        {isPlaying ? 'Pause' : 'Play'}
                    </button>
                    <button onClick={handleReset} className="btn-action-hover" style={actionButtonStyle}>
      ...
      pointerEvents: 'auto'
        ...
        backgroundImage: 'linear-gradient(to right, rgba(255,255,255,0.03) 1px, transparent 1px), linear-gradient(to bottom, rgba(255,255,255,0.03) 1px, transparent 1px)'
                ...
                <svg viewBox="-200 -50 400 450" preserveAspectRatio="xMidYMin meet" style={{
        width: '100%',
        height: '100%',
        zIndex: 2
      }}>
                    ...
                    {/* String */}
                    <line x1="0" y1="0" x2={bobPos.x} y2={bobPos.y} stroke="rgba(255,255,255,0.4)" strokeWidth="4" />
                    
                    {/* Bob (Mass) */}
                    <circle cx={bobPos.x} cy={bobPos.y} r={Math.max(15, mass * 25)} // Radius scales visually with mass
        fill="url(#bobGradient)" filter="drop-shadow(0 10px 20px rgba(10,132,255,0.4))" />
      ...
      gap: '32px'
        ...
        paddingBottom: '12px'
          ...
          marginBottom: '12px'
            ...
            fontWeight: 700
                    ...
                    <input type="range" min="10" max="200" value={length} onChange={e => setLength(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db',
          cursor: 'pointer'
        }} />
          ...
          marginBottom: '12px'
            ...
            fontWeight: 700
                    ...
                    <input type="range" min="0.1" max="3" step="0.1" value={mass} onChange={e => setMass(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#2ecc71',
          cursor: 'pointer'
        }} />
          ...
          marginBottom: '12px'
            ...
            fontWeight: 700
                    ...
                    <select value={gravity} onChange={e => setGravity(Number(e.target.value))} style={{
          width: '100%',
          padding: '12px',
          borderRadius: '8px',
          background: '#1e1e2f',
          ...
          cursor: 'pointer'
                        ...
                        <option value="9.81" style={{
            background: '#1e1e2f',
            color: '#fff'
          }}>Earth (9.81 m/s²)</option>
                        <option value="1.62" style={{
            background: '#1e1e2f',
            color: '#fff'
          }}>Moon (1.62 m/s²)</option>
                        <option value="24.79" style={{
            background: '#1e1e2f',
            color: '#fff'
          }}>Jupiter (24.79 m/s²)</option>
                        <option value="0" style={{
            background: '#1e1e2f',
            color: '#fff'
          }}>Zero Gravity (0 m/s²)</option>
                    </select>
                </div>
          ...
          marginBottom: '12px'
            ...
            color: '#fff',
            fontWeight: 500
          }}>Friction (Damping)</label>
                        <span style={{
            fontSize: '14px',
            color: '#ff375f',
            fontWeight: 700
          }}>{(friction * 1000).toFixed(1)}</span>
                    </div>
                    <input type="range" min="0" max="0.02" step="0.001" value={friction} onChange={e => setFriction(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#ff375f',
          cursor: 'pointer'
        }} />
        ...
        border: '1px solid rgba(52, 152, 219, 0.2)'
          ...
          lineHeight: 1.5
                        ...
                        This simulation computes real-time angular acceleration <span style={{
            fontFamily: 'monospace'
          }}>α = -(g/L)sin(θ)</span> natively using React and SVG. 
                    </p>
                </div>
            </div>
        </div>;
