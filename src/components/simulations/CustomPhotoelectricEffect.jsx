...
const CustomPhotoelectricEffectInner = () => {
  ...
  const updateSimulation = useCallback(time => {
    ...
    if (canvas) {
      ...
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.moveTo(cathodeX, plateY + plateHeight);
      ctx.lineTo(cathodeX, 450);
      ctx.lineTo(anodeX, 450);
      ctx.lineTo(anodeX, plateY + plateHeight);
      ctx.stroke();
      ctx.fillStyle = '#222';
      ctx.fillRect(380, 430, 40, 40);
      ctx.fillStyle = '#fff';
      ctx.font = '14px Arial';
      ...
      ctx.fillStyle = '#333';
      ctx.beginPath();
      ctx.arc(anodeX, 450, 30, 0, Math.PI * 2);
      ctx.fill();
      ctx.strokeStyle = '#fff';
      ctx.lineWidth = 2;
      ctx.stroke();
      ...
      state.photons.forEach(p => {
        ...
        ctx.beginPath();
        ctx.moveTo(p.x, p.y);
        ctx.lineTo(p.x - 10, p.y + Math.sin(p.x) * 4);
        ctx.stroke();
      });
    ...
    fontFamily: "'Inter', sans-serif"
      ...
      boxShadow: '0 20px 40px rgba(0,0,0,0.5)'
        ...
        gap: '6px'
          ...
          <input type="range" min="200" max="850" value={wavelength} onChange={e => setWavelength(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db',
          cursor: 'pointer'
        }} />
        ...
        gap: '6px'
          ...
          <input type="range" min="0" max="100" value={intensity} onChange={e => setIntensity(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db',
          cursor: 'pointer'
        }} />
        ...
        gap: '6px'
          ...
          fontSize: '13px'
            ...
            color: '#3498db',
            fontWeight: 'bold'
          }}>{voltage.toFixed(2)} V</span>
          </div>
          <input type="range" min="-8" max="8" step="0.1" value={voltage} onChange={e => setVoltage(Number(e.target.value))} style={{
          width: '100%',
          accentColor: '#3498db',
          cursor: 'pointer'
        }} />
        ...
        gap: '6px'
          ...
          cursor: 'pointer'
            ...
            {Object.keys(MATERIALS).map(mat => <option key={mat} value={mat} style={{
            background: '#1c1c28',
            color: 'white'
          }}>{mat}</option>)}
          </select>
...
export default function CustomPhotoelectricEffect({
  onBack,
  title
}) {
  return <div style={{
