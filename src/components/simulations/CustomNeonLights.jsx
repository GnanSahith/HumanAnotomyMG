    ...
    const draw = () => {
      ...
      if (s.manualFireTrigger || continuousFired && Math.random() < 0.1) {
        s.electrons.push({
          x: 100,
          y: 100 + Math.random() * 400,
          vx: (2 + voltage / 20) * (dt * 60 || 1),
          // velocity scaled by dt
          energy: voltage
        });
      ...
      gap: '20px'
        ...
        gap: '12px'
          ...
          marginTop: '10px'
            ...
            <input type="range" min="10" max="100" value={voltage} onChange={e => setVoltage(Number(e.target.value))} style={{
            width: '100%',
            accentColor: '#3498db'
          }} />
          </div>
        ...
        gap: '10px'
          ...
          <button onClick={fireElectron} style={{
          ...
          transition: 'all 0.2s',
          width: '100%'
        }} onMouseEnter={e => e.currentTarget.style.background = 'rgba(52, 152, 219, 0.3)'} onMouseLeave={e => e.currentTarget.style.background = 'rgba(52, 152, 219, 0.2)'}>
            Fire Single Electron
          </button>

          <button onClick={() => setContinuousFired(!continuousFired)} style={{
