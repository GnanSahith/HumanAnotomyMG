...
const getInitialMoleculeState = (name) => {
  ...
  switch(name) {
    case 'Carbon Monoxide':
      atoms = [
        { x: cx - 35, y: cy, vx: 0, vy: 0, mass: 12, symbol: 'C', color: '#64748B', radius: 21 },
        { x: cx + 35, y: cy, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 }
      ];
      bonds = [
        { atomA: 0, atomB: 1, restLength: 70, type: 'triple', broken: false }
      ];
      break;
      
    case 'Carbon Dioxide':
      atoms = [
        { x: cx - 75, y: cy, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 },
        { x: cx, y: cy, vx: 0, vy: 0, mass: 12, symbol: 'C', color: '#64748B', radius: 21 },
        { x: cx + 75, y: cy, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 }
      ];
      bonds = [
        { atomA: 0, atomB: 1, restLength: 75, type: 'double', broken: false },
        { atomA: 1, atomB: 2, restLength: 75, type: 'double', broken: false },
        { atomA: 0, atomB: 2, restLength: 150, type: 'virtual', broken: false }
      ];
      break;
    ...
    case 'Water': {
      ...
      atoms = [
        { x: ox, y: oy, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 },
        { x: h1x, y: h1y, vx: 0, vy: 0, mass: 1, symbol: 'H', color: '#F8FAFC', radius: 14 },
        { x: h2x, y: h2y, vx: 0, vy: 0, mass: 1, symbol: 'H', color: '#F8FAFC', radius: 14 }
      ];
      
      const distH1H2 = Math.sqrt(Math.pow(h1x - h2x, 2) + Math.pow(h1y - h2y, 2));
      bonds = [
        { atomA: 0, atomB: 1, restLength: len, type: 'single', broken: false },
        { atomA: 0, atomB: 2, restLength: len, type: 'single', broken: false },
        { atomA: 1, atomB: 2, restLength: distH1H2, type: 'virtual', broken: false }
      ];
      break;
    }
    ...
    case 'Nitrogen':
      atoms = [
        { x: cx - 35, y: cy, vx: 0, vy: 0, mass: 14, symbol: 'N', color: '#3B82F6', radius: 21 },
        { x: cx + 35, y: cy, vx: 0, vy: 0, mass: 14, symbol: 'N', color: '#3B82F6', radius: 21 }
      ];
      bonds = [
        { atomA: 0, atomB: 1, restLength: 70, type: 'triple', broken: false }
    ...
    case 'Oxygen':
      ...
      bonds = [
        { atomA: 0, atomB: 1, restLength: 70, type: 'double', broken: false }
      ];
      break;
    ...
    case 'Ozone': {
      ...
      atoms = [
        { x: o1x, y: o1y, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 },
        { x: o2x, y: o2y, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 },
        { x: o3x, y: o3y, vx: 0, vy: 0, mass: 16, symbol: 'O', color: '#EF4444', radius: 23 }
      ];
      ...
      bonds = [
        { atomA: 0, atomB: 1, restLength: len, type: 'single', broken: false },
        { atomA: 0, atomB: 2, restLength: len, type: 'single', broken: false },
        { atomA: 1, atomB: 2, restLength: distO2O3, type: 'virtual', broken: false }
      ];
      break;
    }
    default:
...
const tick = (state, dt = 1, draggedIndex = null) => {
  ...
  bonds.forEach(bond => {
    ...
    const springK = bond.type === 'virtual' 
      ? 0.08 
      : bond.type === 'triple' 
        ? 0.28 
        : bond.type === 'double' 
...
const calculateEnergies = (atoms, omega) => {
  ...
  return { totalKE, rotKE };
};

const getPhotonColor = (type) => {
  switch(type) {
    case 'microwave': return '#D97706';
    case 'infrared': return '#EF4444';
...
const getPhotonAbbreviation = (type) => {
  switch(type) {
    case 'microwave': return 'MW Photon';
    case 'infrared': return 'IR Photon';
    case 'visible': return 'Visible Photon';
    case 'ultraviolet': return 'UV Photon';
...
const draw = (state, canvas, ctx, draggedIndex = null) => {
  ...
  atoms.forEach((atom, index) => {
    ...
    const radGrad = ctx.createRadialGradient(
      atom.x - atom.radius * 0.25, 
      atom.y - atom.radius * 0.25, 
      atom.radius * 0.08, 
      atom.x, 
      atom.y, 
      atom.radius
  ...
  const { totalKE, rotKE } = calculateEnergies(state.molecule.atoms, state.rotationSpeed);
...
function CustomMoleculesandLightInner({ onBack, title }) {
  ...
  return (
    <div className="flex flex-col h-full text-white select-none overflow-hidden font-sans">
      {/* Simulation Header */}
      ...
      <div style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none' }}>
        ...
        <div className="w-full xl:w-80 flex flex-col gap-5 shrink-0 xl:overflow-y-auto xl:pr-1">
          ...
          <div className="border border-slate-800/80 backdrop-blur-md rounded-2xl p-5 shadow-lg flex flex-col gap-4" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}>
            ...
            <div className="flex gap-2">
              <button
                ...
                className={`flex-1 flex justify-center items-center gap-2 py-3 px-4 rounded-xl font-bold transition-all ${
                  isPlaying 
                    ? 'bg-red-500/20 text-red-300 border border-red-500/30 hover:bg-red-500/30' 
                    : 'bg-emerald-500/25 text-emerald-300 border border-emerald-500/35 hover:bg-emerald-500/35'
                }`}
              >
              ...
              <button
                onClick={handleStep}
                className="px-4 py-3 rounded-xl border border-slate-700/40 text-slate-200 hover: font-semibold text-sm transition-all" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}
              ...
              <button
                onClick={handleReset}
                className="px-4 py-3 rounded-xl border border-slate-700/40 text-slate-300 hover: transition-all" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}
                  ...
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border text-left transition-all ${
                      isSelected 
                        ...
                        : 'bg-transparent border-transparent hover: text-slate-400 hover:text-white'
                    }`}
                  >
                    <div className={`p-2 rounded-lg /60 border ${
                      isSelected ? `border-slate-600` : 'border-slate-800'
          ...
          <div className="border border-slate-800/80 backdrop-blur-md rounded-2xl p-5 shadow-lg flex flex-col gap-4" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}>
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${
                  lightOn
                    ...
                    : ' text-slate-400 border-slate-700/50 hover:'
                }`}
              >
            ...
            <div className="flex gap-1.5 p-1 /60 rounded-lg border border-slate-800">
              <button
                onClick={() => setEmissionStyle('continuous')}
                className={`flex-1 py-1.5 rounded-md text-xs font-bold transition-all ${
                  emissionStyle === 'continuous'
                    ? ' text-purple-400 border border-slate-800 shadow'
            ...
            {emissionStyle === 'continuous' ? (
              ...
              /* Single Photon Pulse Button */
              <button
                onClick={handleFirePhoton}
                className="w-full py-2.5 bg-purple-600 hover:bg-purple-500 text-white font-bold text-sm rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] border border-purple-500/20 shadow-md flex justify-center items-center gap-1.5"
              >
                <Zap size={16} />
                Fire Single Photon
              </button>
        ...
        <div className="flex-1 flex flex-col gap-5 min-w-0">
          <div className="border border-slate-800/80 backdrop-blur-md rounded-3xl p-4 shadow-xl flex-1 flex flex-col min-h-0 relative" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}>
            ...
            <div style={{ position: 'absolute', inset: 0, pointerEvents: 'auto' }}>
              <canvas
                ...
                className="absolute inset-0 w-full h-full block object-contain cursor-grab active:cursor-grabbing"
              />
            </div>
            ...
            <div className="mt-4 p-4 rounded-xl /60 border border-slate-850 flex items-start gap-3 shrink-0">
              <div 
                ...
                style={{ borderColor: activeLight.color + '40', color: activeLight.color, backgroundColor: activeLight.color + '15' }}
              >
                <HelpCircle size={18} />
              </div>
        ...
        <div className="w-full xl:w-96 flex flex-col gap-5 shrink-0 xl:overflow-y-auto xl:pl-1">
          ...
          <div className="border border-slate-800/80 backdrop-blur-md rounded-2xl p-5 shadow-lg flex flex-col gap-4 flex-1 min-h-[300px]" style={{ background: 'rgba(20, 20, 30, 0.8)', border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(12px)', borderRadius: '16px', color: 'white' }}>
            {/* Tabs Selector */}
            <div className="flex gap-2 border-b border-slate-800 pb-2.5">
              <button
                onClick={() => setInfoTab('interaction')}
                className={`text-xs font-semibold pb-1 relative transition-all ${
                  infoTab === 'interaction' ? 'text-purple-400' : 'text-slate-500 hover:text-slate-350'
                }`}
              >
                Active Description
                {infoTab === 'interaction' && (
              ...
              <button
                ...
                className={`text-xs font-semibold pb-1 relative transition-all ${
                  infoTab === 'details' ? 'text-purple-400' : 'text-slate-500 hover:text-slate-350'
                }`}
              >
                Chemical Details
                {infoTab === 'details' && (
...
export default function CustomMoleculesandLight({ onBack, title }) {
    return (
