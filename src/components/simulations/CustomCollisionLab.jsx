...
import { Play, Pause, RotateCcw, ArrowLeft, Sliders, Sparkles, HelpCircle, Volume2, VolumeX, Eye, Info, RefreshCw } from 'lucide-react';
...
const PRESETS = [
    {
        ...
        mode: '1d',
        elasticity: 1.0,
        m1: 2.0, x1: 200, y1: 225, vx1: 3.0, vy1: 0,
        m2: 2.0, x2: 600, y2: 225, vx2: -3.0, vy2: 0,
    ...
    },
    {
        ...
        mode: '1d',
        elasticity: 0.0,
        m1: 3.0, x1: 200, y1: 225, vx1: 4.0, vy1: 0,
        m2: 1.0, x2: 500, y2: 225, vx2: 0, vy2: 0,
    ...
    },
    {
        ...
        mode: '1d',
        elasticity: 1.0,
        m1: 8.0, x1: 200, y1: 225, vx1: 2.5, vy1: 0,
        m2: 0.5, x2: 500, y2: 225, vx2: 0, vy2: 0,
    ...
    },
    {
        ...
        mode: '2d',
        elasticity: 1.0,
        m1: 2.5, x1: 250, y1: 160, vx1: 3.5, vy1: 1.0,
        m2: 2.5, x2: 550, y2: 290, vx2: -2.5, vy2: -1.0,
    ...
    },
    {
        ...
        mode: '2d',
        elasticity: 0.4,
        m1: 3.0, x1: 280, y1: 150, vx1: 3.0, vy1: 1.5,
        m2: 1.5, x2: 500, y2: 300, vx2: -1.0, vy2: -2.0,
        desc: 'Off-center collision with 40% elasticity. Kinetic energy is partially converted.'
    }
];

function CustomCollisionLabInner({ onBack, title }) {
    // ==========================================
    ...
    const physicsStateRef = useRef({
        ...
        ball2: {
            ...
            radius: getRadiusFromMass(2.5),
            mass: 2.5,
            color: '#fb7185', // rose-400 red
            trail: []
        },
        draggedBall: null,   // null, 1, or 2
    ...
    const updatePhysics = (dt) => {
        ...
        for (let step = 0; step < subSteps; step++) {
            ...
            const walls = { left: 0, right: 800, top: 0, bottom: 450 };
            const eWall = 1.0;
                    ...
                    if (relSpeed > 0.15) {
                        ...
                        for (let i = 0; i < numSparks; i++) {
                            ...
                            sparksRef.current.push({
                                x: midX,
                                y: midY,
                                vx: sv.vx + (state.ball1.vx + state.ball2.vx) * 0.4, // carry system momentum bias
        ...
        if (state.showTrails && tickCountRef.current % 2 === 0) {
            state.ball1.trail.push({ x: state.ball1.x, y: state.ball1.y });
            if (state.ball1.trail.length > 45) state.ball1.trail.shift();

            state.ball2.trail.push({ x: state.ball2.x, y: state.ball2.y });
    ...
    const drawFrame = () => {
        ...
        if (vectorType === 'velocity' || vectorType === 'both') {
            ...
            if (Math.abs(b1.vx) > 0.02 || Math.abs(b1.vy) > 0.02) {
                ...
                drawArrow(ctx, b1.x, b1.y,
                    b1.x + b1.vx * vScale,
                    b1.y + b1.vy * vScale,
                    '#10b981', 2.2, `${speed1.toFixed(1)}m/s`);
            }
            if (Math.abs(b2.vx) > 0.02 || Math.abs(b2.vy) > 0.02) {
                const speed2 = Math.sqrt(b2.vx * b2.vx + b2.vy * b2.vy);
                drawArrow(ctx, b2.x, b2.y,
                    b2.x + b2.vx * vScale,
                    b2.y + b2.vy * vScale,
                    '#10b981', 2.2, `${speed2.toFixed(1)}m/s`);
            }
        }
        ...
        if (vectorType === 'momentum' || vectorType === 'both') {
            ...
            if (pMag1 > 0.02) {
                drawArrow(ctx, b1.x, b1.y + offset,
                    b1.x + px1 * pScale,
                    b1.y + py1 * pScale + offset,
                    '#f59e0b', 2.2, `${pMag1.toFixed(1)}kg·m/s`);
            }
            if (pMag2 > 0.02) {
                drawArrow(ctx, b2.x, b2.y + offset,
                    b2.x + px2 * pScale,
                    b2.y + py2 * pScale + offset,
                    '#f59e0b', 2.2, `${pMag2.toFixed(1)}kg·m/s`);
            }
        }
    };
    ...
    const loadPreset = (preset) => {
        ...
        initialKERef.current = getKineticEnergy(preset.m1, preset.vx1, preset.vy1) +
                               getKineticEnergy(preset.m2, preset.vx2, preset.vy2);
    ...
    const togglePlay = () => {
        ...
        if (!state.isPlaying) {
            ...
            initialKERef.current = getKineticEnergy(state.ball1.mass, state.ball1.vx, state.ball1.vy) +
                                   getKineticEnergy(state.ball2.mass, state.ball2.vx, state.ball2.vy);
        }
    ...
    const handleCanvasMouseDown = (evt) => {
        ...
        const checkVectorHead = (ball, index) => {
            ...
            if (dx * dx + dy * dy < 144) { // Click radius of 12px around head
                ...
                <div style={{
                    ...
                    <div className="flex border-b border-slate-800 /50">
                        <button
                            ...
                            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1.5 transition ${
                                activeTab === 'setup' ? 'text-sky-400 border-b-2 border-sky-400 ' : 'text-slate-400 hover:text-slate-300'
                            }`}
                        >
                            <Sparkles size={16} /> Setup
                        </button>
                        <button
                            onClick={() => setActiveTab('ball1')}
                            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1.5 transition ${
                                activeTab === 'ball1' ? 'text-sky-400 border-b-2 border-sky-400 ' : 'text-slate-400 hover:text-slate-300'
                            }`}
                        >
                            <Sliders size={16} className="text-sky-400" /> Ball 1
                        </button>
                        <button
                            onClick={() => setActiveTab('ball2')}
                            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1.5 transition ${
                                activeTab === 'ball2' ? 'text-rose-400 border-b-2 border-rose-400 ' : 'text-slate-400 hover:text-slate-300'
                            }`}
                        >
                        ...
                        <button
                            ...
                            className={`flex-1 py-3 text-xs font-semibold flex flex-col items-center gap-1.5 transition ${
                                activeTab === 'info' ? 'text-amber-400 border-b-2 border-amber-400 ' : 'text-slate-400 hover:text-slate-300'
                            }`}
                        >
                        ...
                        {activeTab === 'ball1' && (
                            <div className="flex flex-col gap-5">
                                ...
                                {mode === '2d' ? (
                                    ...
                                    <div className="p-3 /45 rounded-lg border border-slate-800/80 text-[11px] text-slate-500">
