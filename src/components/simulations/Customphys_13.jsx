    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                <div style={{ position: 'relative', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(255, 255, 255, 0.1)', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.5)' }}>
                    <div style={{ position: 'absolute', top: '16px', left: '16px', color: '#94a3b8', fontSize: '12px', fontWeight: '500', opacity: 0.8, pointerEvents: 'none', zIndex: 10 }}>
            ...
            <aside style={{
                ...
                <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: '#f8fafc', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', paddingBottom: '10px' }}>Spring Parameters</h3>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                        <span style={{ color: '#94a3b8' }}>Mass</span>
                        <span style={{ color: '#3498db', fontWeight: 'bold' }}>{mass}g</span>
                    </div>
                    <input 
                        type="range" min="50" max="300" step="10" 
                        value={mass} 
                        onChange={e => setMass(Number(e.target.value))} 
                        style={{ cursor: 'pointer', width: '100%', accentColor: '#3498db' }}
                    />
                </div>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                        <span style={{ color: '#94a3b8' }}>Spring Stiffness</span>
                        <span style={{ color: '#2ecc71', fontWeight: 'bold' }}>{(stiffness * 1000).toFixed(0)}</span>
                    </div>
                    <input 
                        type="range" min="0.001" max="0.1" step="0.001" 
                        value={stiffness} 
                        onChange={e => setStiffness(Number(e.target.value))} 
                        style={{ cursor: 'pointer', width: '100%', accentColor: '#2ecc71' }}
                    />
                </div>
                
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                        <span style={{ color: '#94a3b8' }}>Damping (Friction)</span>
                        <span style={{ color: '#ef4444', fontWeight: 'bold' }}>{(damping * 100).toFixed(0)}%</span>
                    </div>
                    <input 
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '13px' }}>
                        ...
                        <span style={{ color: '#e67e22', fontWeight: 'bold' }}>{gravity.toFixed(1)}g</span>
                    </div>
                    <input 
                        type="range" min="0.1" max="3" step="0.1" 
                        value={gravity} 
                        onChange={e => setGravity(Number(e.target.value))} 
                        style={{ cursor: 'pointer', width: '100%', accentColor: '#e67e22' }}
                    />
                </div>
            </aside>
        </div>
...
export default function Customphys_13({ onBack, title }) {
    return (
