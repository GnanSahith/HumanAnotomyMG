...
export default function ChemistrySimulationView({ onBack }) {
    ...
    const filteredSimulations = chemistrySimulations.filter(sim => 
    ...
    return (
        <div className="maths-sim-container fade-in" style={{ 
            ...
            {activeSimulation ? (
                <div style={{ padding: '0 24px 24px 24px', display: 'flex', flexDirection: 'column', flex: 1 }} className="fade-in">
                ...
                <div style={{ padding: '0 24px', display: 'flex', flexDirection: 'column', gap: '48px' }}>
                    {Object.entries(
                        filteredSimulations.reduce((acc, sim) => {
                            ...
                            acc[sim.category].append ? acc[sim.category].push(sim) : acc[sim.category].push(sim);
                            return acc;
                        }, {})
                    ).map(([category, sims]) => (
                        <div key={category} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', borderBottom: '1px solid rgba(255,255,255,0.1)', paddingBottom: '12px' }}>
                                <div style={{ width: '4px', height: '24px', background: '#ff375f', borderRadius: '4px' }}></div>
                            ...
                            <div style={{
                                ...
                                {sims.map((sim, index) => {
                                    ...
                                    return (
                                        ...
                                        >
                                        <div style={{ height: '180px', position: 'relative', overflow: 'hidden', background: 'linear-gradient(135deg, rgba(255,55,95,0.05) 0%, rgba(255,55,95,0.2) 100%)' }}>
                                            <div style={{ 
                                                position: 'absolute', top: '-20%', right: '-10%', 
                                                width: '150px', height: '150px', borderRadius: '50%',
                                                background: 'radial-gradient(circle, rgba(255,55,95,0.4) 0%, rgba(0,0,0,0) 70%)',
                                                filter: 'blur(20px)'
                                            }}></div>
                                            <div style={{ 
                                                position: 'absolute', bottom: '-20%', left: '-10%', 
                                                width: '200px', height: '200px', borderRadius: '50%',
                                                background: 'radial-gradient(circle, rgba(255,214,10,0.2) 0%, rgba(0,0,0,0) 70%)',
                                                filter: 'blur(30px)'
                                            }}></div>
                                            <div style={{
                                                ...
                                                <FlaskConical size={64} color="rgba(255,55,95,0.8)" style={{ filter: 'drop-shadow(0 0 10px rgba(255,55,95,0.5))' }} />
                                            </div>
